...
def perform_handshake():
    ...
    if (message_received == "abc\r\n"):
        print("Hand Shake Success: received ABC response from Arduino")
    else:
        print("Hand Shake Failed: No Response from Arduino")
...
def rpi_digital_low_input_test():
    try:
        GPIO.setmode(GPIO.BCM)
...
def rpi_switches_test():
    ...
    try:
        ...
        print("RPI digital GPIO input test failed on pin " + str(power_pin))
        print(type(error))
        print(error)
    ...
    while True:
        try:
            ...
            if (is_escape(power_pin)):
                return
            elif (is_arduino_switch(power_pin) or power_pin == '-1'):
                break
                
        except (ValueError):
            # Set power pin to default value if no pin was given or incorrect pin
            power_pin = -1
            break
...
def display_options_menu():
    ...
    print("5:   RPI digital IO as INPUT")
    print("6:   Arduino Switches")
    print("7:   Arduino Analog IO as INPUT")
    print("8:   SDI12 Sensor Ports")
    print("9:   Get onboard DC voltage via VSENSE, requires vsense\n     and jumper (switch 9) set correctly")
    print("10:  Check Arduino Runtime Since Last Communication")
