...
pht_grange_3_mo_su = Phant(publicKey=public_qal_moisture_suction, fields=field_name ,privateKey=private_qal_moisture_suction,baseUrl=nectar_address)
...
# ------------------------------- below goes to electrochem_o2  --------------------------------------------
    ...
    if save_to_file: fid.write(delimiter+msg.rstrip())
    current_read=msg.split(' ')[0:-1]
    grange_3_luo2_dry['dluo7'] = float(current_read[-2])
    grange_3_luo2_dry['dlupe7'] = float(current_read[-4])
    #grange_3_luo2_dry['dlut0'] = float(current_read[-6])
    grange_3_luo2_dry['dlup7'] = float(current_read[-8])
    ...
    if save_to_file: fid.write(delimiter+msg.rstrip())
    current_read=msg.split(' ')[0:-1]
    grange_3_luo2_dry['wluo7'] = float(current_read[-2])
    grange_3_luo2_dry['wlupe7'] = float(current_read[-4])
    #grange_3_luo2_dry['wlut7'] = float(current_read[-6])
    grange_3_luo2_dry['wlup7'] = float(current_read[-8])
