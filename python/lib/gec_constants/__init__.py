...
def swcc_fredlund_xing_1994(**kwargs):
    ...
    if arg['plot']:
        ...
        plt.ylabel('GRAVIMETRIC WATER CONTENT (g)',
    ...
    for i, k in enumerate(np.atleast_1d(arg['vwc'])):
        ...
        if k >= arg['por']:
            ...
            while abs(psi_0-psi_1) > 0.0001:
                ...
                log_e = np.log(e_plus)
                dw_dpsi_0 = - arg['mf']*arg['nf']*psi_on_af**(-1+arg['nf']) * arg['por'] * (1 - log_on_log) * log_e ** (
                    -1-arg['mf']) / arg['af'] / e_plus - arg['por'] * log_e**(-arg['mf'])/arg['hr']/(1+psi_0/arg['hr'])/np.log(1+1.e6/arg['hr'])
...
def latlon_two_ponts_to_delta_xy_m(latlon1, latlon2):
    ...
    dx = (latlon2[1]-latlon1[1])*40000 * \
            np.cos((latlon1[0]+latlon2[0])*np.pi/360)/360*mPkm
