...
from .types import LocationMetadata
from .utils import validate_decimal_degree_coordinates

__all__ = [
    "LocationMetadata",
    "get_historical",
    "get_location_meta",
    "extract_timeseries_dataframe",
]
...
def get_historical(coordinates, start_date, end_date, hourly=None, daily=None, tz=None, settings=None, timeout=(5, 30)):  # pylint: disable=line-too-long
    ...
    if isinstance(coordinates, tuple):
        ...
        for coordinate in coordinates:
            validate_decimal_degree_coordinates(coordinate)
    else:
        raise TypeError(
...
def extract_timeseries_dataframe(point_data, freq):
    ...
    df.columns = _pd.MultiIndex.from_tuples(
        [(column, data_units.get(column, "")) for column in df.columns],
        names=["field", "unit"]
    )
    return df
