...
- [`group_timeseries_by_key`](./thingsboard_api.tb_rest_api.md#function-group_timeseries_by_key): Converts timestamped records into a ThingsBoard timeseries response dictionary.
- [`group_timeseries_by_ts`](./thingsboard_api.tb_rest_api.md#function-group_timeseries_by_ts): Groups timestamped key-value readings into records by timestamp.
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L915"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>

## <kbd>function</kbd> `group_timeseries_by_ts`

```python
group_timeseries_by_ts(data, flatten=False, latest_first=False)
```

Groups timestamped key-value readings into records by timestamp.

Readings with the same timestamp across different keys are merged into
a single record. The resulting records are sorted by timestamp.

Missing keys:  
    Only keys with a reading at a given timestamp are included in that
    record.


**Args:**

- <b>`data`</b> (dict or list): The input data to process. This accepts either:
    - A dictionary of the form:
        ...
        ```
    - A list matching the exact structure and format returned by this function.

- <b>`flatten`</b> (bool, optional): If True, keys are spread to the top level of
    each record. Missing keys are omitted. Defaults to False.
- <b>`latest_first`</b> (bool, optional): If ``True``, return records from latest to earliest
    timestamp. If ``False``, return records chronologically from earliest
    to latest. Defaults to False.
...
- <b>`TypeError`</b>: If ``data`` is not a dictionary or list.

> [!WARNING] 
> UserWarning: If `ts` is missing from any time-series data record.
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L1025"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>

## <kbd>function</kbd> `group_timeseries_by_key`

```python
group_timeseries_by_key(data)
```

Converts timestamped records into a ThingsBoard timeseries response dictionary.

This function takes a list of timestamped records and groups the readings
by key into the same dictionary structure returned by the ThingsBoard
time-series request API.

Each key maps to a list of timestamped readings, where each reading
contains a ``ts`` timestamp and a ``value``.

Missing keys:  
    A record is only included if the timestamp for the key exists.


**Args:**

- <b>`data`</b> (list): The input records to process. This must match the exact
    structure and format returned by :func:`group_timeseries_by_ts`.


**Returns:**

- <b>`dict`</b>: A dictionary containing the time series grouped by key, matching
    the format returned by the ThingsBoard time-series request API.


**Raises:**

- <b>`TypeError`</b>: If ``data`` is not a list.
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L1077"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L1137"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L1172"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L814"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>
...
- <b>`keys`</b> (tuple|list|str, optional): Telemetry keys to fetch.
    ...
    Defaults to None.
- <b>`order_by_desc`</b> (bool, optional): Whether to order the returned time-series
    data by timestamp in descending order (``DESC``). When ``False``, data
    is ordered in ascending order (``ASC``). Defaults to ``True``.
    ``orderBy`` is only supported by ThingsBoard version 3.2 and later.
...
- <b>`agg_params`</b> (dict, optional): Parameters used to configure time-series
...
- <b>`timeout`</b> (int | float | tuple, optional): Request timeout.
    ...
    `(connect, read)` tuple to set them separately.
    Defaults to None, which applies the internal `DEFAULT_TIMEOUT`.
**api_params: Additional parameters supported by the ThingsBoard
    timeseries API, excluding aggregation parameters. These are passed
    through to the API unchanged and may override existing parameters.

    > [!WARNING] 
    > Use with caution; this is not intended for normal use.
...
- <b>`ValueError`</b>: If `agg_params` is not a non-empty dictionary, if `agg` or
    `interval` is missing from `agg_params`, or if `startTs` or `endTs`
    is missing when aggregation is requested.
- <b>`TypeError`</b>: If `interval` or `limit` has an invalid type.
- <b>`OverflowError`</b>: If `limit` exceeds the maximum supported value.

> [!WARNING] 
> UserWarning: If `limit` is reached, indicating that the returned
> time-series data may be truncated.
...
- https://thingsboard.io/docs/reference/rest-api/#get-time-series-range
...
<a href="../../../../python/lib/thingsboard-api/src/thingsboard_api/tb_rest_api.py#L766"><img align="right" style="float:right;" src="https://img.shields.io/badge/-source-cccccc?style=flat-square" /></a>
