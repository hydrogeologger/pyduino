...
def convert_to_dataframes(data, keys=None, to_numeric=None, drop_ts=True, verbose=False):  # pylint: disable-next=line-too-long
    # type: (dict[str, list[dict[str, _Any]]], list|tuple|set|_KeysView|None, bool|list|tuple|set|_KeysView|None, _Optional[bool], _Optional[bool]) -> dict[str, _pd.DataFrame]
    ...
    Args:
        ...
        keys (tuple|list|set, optional): Limit data to specified telemetry keys only.
            Defaults to None.
        to_numeric (bool|tuple|list|set, optional): Convert values of given keys
    ...
    for key in keys:
        ...
        try:
            ...
            if to_numeric and (isinstance(to_numeric, bool) or key in to_numeric):
...
def convert_to_dataframe(data, keys=None, to_numeric=None, drop_ts=True, verbose=False):  # pylint: disable-next=line-too-long
    # type: (dict[str, list[dict[str, _Any]]], _Optional[tuple|list], bool|list|tuple|set|_KeysView|None, _Optional[bool], _Optional[bool]) -> _pd.DataFrame
    ...
    """
    return dataframes_to_dataframe(convert_to_dataframes(data=data, keys=keys,
...
def dataframes_to_dataframe(dataframes, clean=True):
    # type: (dict[str, _pd.DataFrame], _Optional[bool]) -> _pd.DataFrame
...
def dataframe_to_dataframes(dataframe, clean=True):
    # type: (_pd.DataFrame, _Optional[bool]) -> dict[str, _pd.DataFrame]
    """
    ...
    Returns:
        dict[str,pandas.DataFrame]: A dictionary containing dataframe object with
