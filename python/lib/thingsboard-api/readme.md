...
```shell
pip install "git+https://github.com/hydrogeologger/pyduino.git@pylib/thingsboard-api#egg=thingsboard-api&subdirectory=python/lib/thingsboard-api"

# or

pip install "thingsboard-api @ git+https://github.com/hydrogeologger/pyduino.git@pylib/thingsboard-api#subdirectory=python/lib/thingsboard-api"
```
...
- <https://codemia.io/knowledge-hub/path/how_can_i_install_from_a_git_subdirectory_with_pip>
- <https://gist.github.com/Josverl/e48f8204579a16dcff94cfeed500c4b4>
...
device.fetch_timeseries_keys()  # Retrieve all timeseries keys for device
...
# NOTE: See doc for additional dataframe conversion parameters,
# Collate all telemetry key into single dataframe (multiple key per dataframe)
data_frame = tb_pandas.convert_to_dataframe(data, type_conversion=False)

# Collate each telemetry key in own dataframe (one dataframe per key)
data_frames = tb_pandas.convert_to_dataframes(data, type_conversion=False)
```
...
For a breakdown of all modules and functions, please see the [API Reference](./docs/readme.md).
