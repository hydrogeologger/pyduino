...
and contains thingsboard specific post processing and pandas library helper methods.
...
or
pip install -e . # if you are in the ...\lib\thingsboard-api directory
```
...
device.get_keys()
result = device.get_data(startTs=tb_pandas.datetime(2022, 6, 1),
...
device = tb_pandas.Device(account=account,
                          ...
                          device_id="55f0d405-248d-eb11-8a6b-9b13d3e7fe2e")
device.get_keys()
data = device.get_data(startTs=tb_pandas.datetime(2022, 6, 1),
                       endTs=tb_pandas.datetime.now(),
                       keys=None,
                       limit=1000, tz_offset=0)
