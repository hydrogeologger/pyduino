    ...
    def get_timeseries(self,
        ...
        Args:
            ...
            keys (tuple|list|str, optional): Telemetry keys to fetch.
                ...
                Defaults to None.
            order_by_desc (bool, optional): Whether to order the returned time-series
        ...
        # Build "limit" param
        MAX_LIMIT = 50000  # Limit as observed on GUI
        try:
        ...
        if interval:
            if isinstance(interval, timedelta):
                ...
                isinstance(interval, str) and interval.isnumeric()
                or (
                    not isinstance(interval, bool)
                    and isinstance(interval, (int, long))
                )
            ):
        ...
        params["orderBy"] = "DESC" if order_by_desc else "ASC"
...
def group_timeseries_by_ts(data, flatten=False, latest_first=False):
    # type: (dict[str, list[dict]]|list[dict], bool, bool) -> list[dict]
    ...
    Args:
        data (dict or list): The input data to process. This accepts either:
        ...
        latest_first (bool, optional): If ``True``, return records from latest to earliest
    ...
    if isinstance(data, dict):
        # json dict as thingsboard response
        ...
        # Data is grouped by timestamp
        for record in data:
            ts = record.get("ts")
            if ts is None:
                _warnings.warn("Missing `ts` field in `data`.",
                               UserWarning, stacklevel=2)
            # `values` is `record` for flatten structure
            values = record.get("values", record)  # type: dict
            for key, value in values.items():
                # ts key will equal to record ts for flattend structure
                if key == "ts" and ts == value:
                    continue
                grouped[ts][key] = value
    else:
...
def group_timeseries_by_key(data):
    ...
    Args:
        data (list): The input records to process. This must match the exact
            structure and format returned by :func:`group_timeseries_by_ts`.

    Returns:
        dict: A dictionary containing the time series grouped by key, matching
    ...
    for record in data:
        ts = record.get("ts")
        if ts is None:
            _warnings.warn("Missing `ts` field in `data` record.",
                           UserWarning, stacklevel=2)
            continue
        # `values` is `record` for flatten structure
        values = record.get("values", record)  # type: dict
        for key, value in values.items():
            ...
            if key not in grouped:
                grouped[key] = []
            grouped[key].append({"ts": ts, "value": value})
