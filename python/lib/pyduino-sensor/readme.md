...
### Initialization
```python
Rain(name, pin, debounce, resolution)
```
### Params
...
* resolution (float) - Resolution of tip bucket in millimeteres (mm), Default: 0.2 mm
...
get_resolution()
```
Returns the resolution per tip of the rain bucket, as configured during initialization.

### get_cumulative()
```python
get_cumulative(reset=True)
```
Getter for cumulative rain in millilitres (mm). Will reset counter.
