"""
This is an MQTT client publishing helper module providing file base \
    queue redundancy support.
...
def generate_filename(filename=None):
    ...
    if filename is None or filename == "":
        parent_filename = os.path.basename(_main_script_path).rsplit('.', 1)[0]
...
def load_queue_from_file(filename):
    ...
    if os.path.isfile(filename):
        with _do_open(filename, "r", encoding='utf-8') as json_file:
            try:
...
def save_queue_to_file(filename, json_queue, start=None, end=None):
    ...
    if not json_queue:
        ...
        with _do_open(filename, "w", encoding="utf-8"):
            pass  # Do nothing, empty the file
    else:
        with _do_open(filename, "w", encoding="utf-8") as json_file:
