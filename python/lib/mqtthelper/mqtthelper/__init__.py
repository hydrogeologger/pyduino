...
def _do_open(file, mode, encoding, *args, **kwargs):
    # type: (str|bytes|_os.PathLike, str, str|None, ..., ...) -> _IO[any]
...
def generate_filename(filename=None):
    ...
    Returns:
        str: Filename including extension. i.e mqtt_queue_callingscriptname.json.
    """
    if filename is not None:
        filename = filename.strip()
    if filename is None or filename == "":
        parent_filename = _os.path.basename(
            _main_script_path).rsplit('.', 1)[0]
...
def is_json_object(object_):
    ...
    Returns:
        Literal[0, 1, 2]: Literal value representing valid json object.
...
def is_json_string(text):
    ...
    Returns:
        Literal[0, 1, 2]: Literal value representing valid json string.
...
def publish_mqtt_queue(client, topic, json_queue,
    ...
    # type: (_mqtt_client.Client, str, dict|list[dict]|str, int|float|None, bool|None, bool|None, int|None) -> _mqtt_client.MQTTMessageInfo # pylint: disable=line-too-long
...
def publish_to_thingsboard(client, payload, ts=None,
                           ...
                           debug=False,
                           max_payload_bytes=MQTTHELPER_NETTY_MAX_PAYLOAD_SIZE):
    # type: (_mqtt_client.Client, dict|list[dict]|str, int|None, bool|None, int|float|None, str|None, bool|None, bool|None, int|None) -> _mqtt_client.MQTTMessageInfo # pylint: disable=line-too-long
    ...
    if json_queue or file_queue_size:
        save_queue_to_file(filepath=json_filename, json_queue=json_queue)
    return publish_result
