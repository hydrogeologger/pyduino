...
"""As per MQTT Broker MAX Payload Size Setting."""
...
def is_json_object(object_):
    ...
    Returns:
        Literal[0, 1, 2]: Invalid - 0 : MQTTHELPER_ERR_INVALID. \
...
def publish_mqtt_queue(client, topic, json_queue,
    ...
    # type: (_mqtt_client.Client, str, dict|list[dict]|str, int|float, bool, bool) -> _mqtt_client.MQTTMessageInfo # pylint: disable=line-too-long
    ...
    while (len(_json.dumps(json_queue[json_start:json_end]).encode("utf-8"))
...
def publish_to_thingsboard(client, payload, ts=None,
                           ...
                           debug=False):
    # type: (_mqtt_client.Client, dict|list[dict]|str, int|None, bool, int|float, str|None, bool, bool) -> _mqtt_client.MQTTMessageInfo # pylint: disable=line-too-long
