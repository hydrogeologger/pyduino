...
class phant_downloader:
    ...
    def __init__(self,web_addr='Nan',file_path_abs='',download=False,**kwargs ):
	...
	import numpy as np
        ...
        self.file_path_abs=os.getcwd() +'/' +file_path_abs
        self.file_dir=os.path.dirname(self.file_path_abs)+'/'
        if download:
            print 'getting data from ' + web_addr
            response = urllib2.urlopen(web_addr)
            html = response.read()
            text_file = open(file_path_abs, "w")
            print 'save data to ' + self.file_path_abs 
            text_file.write(html)
            text_file.close()
        else:
            print 'just creat object without downloading file ' + web_addr +' ' +self.file_path_abs
